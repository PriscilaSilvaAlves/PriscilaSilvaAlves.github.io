...
const Placas = () => {
 ...
 const handleFileChange = async (event) => {
  console.log("Entruo na função handleFileCHange");
  ...
  reader.onload = async () => {
    ...
    try {
      ...
      console.log('Texto completo:', JSON.stringify(fullText));
      const testLinhas = fullText.split('\n');
      console.log('Linhas de teste após split por \\n:', testLinhas);

      const linhas = fullText
        .split(/(?=Oferta|Aproveite)/g)
        .map(l => l.trim())
        .filter(l => l.length > 0 && !l.toLowerCase().includes('tipo'));
      console.log('Linhas após split e filtro:', linhas);

      const novasPlacas = linhas.map((linha, index) => {
        ...
        if (precoPromocional.includes(',')) {
          const [inteiro, centavos] = precoPromocional.split(',');
          precoReal = parseInt(inteiro);
          precoCentavos = parseInt(centavos);
        }
        ...
        return {
          ...
          precoReal,
          precoCentavos,
          dataInicial: dataInicial ? formatarData(dataInicial) : '',
          dataFinal: dataFinal ? formatarData(dataFinal) : ''
        };
      }).filter(p => p !== null);
      console.log('Novas placas extraídas do PDF:', novasPlacas);
      setPlacas(prev => {
        const updated = [...prev, ...novasPlacas];
        console.log('Estado placas atualizado:', updated);
        return updated;
      });
    } catch (error) {
